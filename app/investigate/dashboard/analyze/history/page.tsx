        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {analyses && analyses.length > 0 && (
              <Select
                onValueChange={(value) => {
                  router.push(`/investigate/dashboard/analyze/history/${value}`);
                }}
              >
                <SelectTrigger className="w-[220px]">
                  <SelectValue placeholder="Jump to analysis..." />
                </SelectTrigger>
                <SelectContent>
                  {analyses.map((analysis) => (
                    <SelectItem key={analysis._id} value={analysis._id}>
                      <div className="flex flex-col gap-0.5">
                        <span className="font-medium truncate max-w-[180px]">{analysis.fileName}</span>
                        <span className="text-xs text-muted-foreground">
                          {new Date(analysis.timestamp).toLocaleDateString()} • {analysis.rowCount} rows
                        </span>
                      </div>
                    </SelectItem>
                  ))}
  ...
  return (
    <Card className="hover:shadow-md transition-shadow">
      <CardContent className="p-0">
        <Link href={`/investigate/dashboard/analyze/history/${analysis._id}`}>
          <div className="p-6">
